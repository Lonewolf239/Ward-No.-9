...
KINDS_BY_FLOOR = {
    "upper": ["ward", "office", "morgue", "cafeteria", "plain",
              "bath", "pharmacy", "archive", "dayroom", "nurse", "treatment",
              "entrance", "exit", "unlocker", "corridor"],
    "basement": ["boiler", "storage", "cell", "morgue", "plain",
                 "workshop", "pump", "laundry", "archive",
...
DOOR_SIDES = ("N", "S", "E", "W")
DOOR_KINDS = ("passage", "door", "broken", "random")
PI, HALF, TAU34 = math.pi, math.pi / 2, 3 * math.pi / 2


class RoomModel(FurnitureGrid):
    ...
    def _cell_takes_furniture(self, x, y):
        return (x, y) not in self.door_cell_set()
    ...
    def resize(self, w, h):
        ...
        self.furniture = [f for f in self.furniture
                          if self.in_bounds(int(f[1]), int(f[2]))]
    ...
    def bar_enclosed_cells(self):
        if not self.doors:
            return set()
        start = tuple(next(iter(self.doors.values()))["cell"])
        return bar_enclosed(self.cells, self.w, self.h, self.furniture, start)
    ...
    def all_errors(self):
        return self.connectivity_errors() + self.content_errors() + self.window_errors()
...
def flood_cells(open_cells, start):
    ...
    while stack:
        ...
        for dx, dy in ((1, 0), (-1, 0), (0, 1), (0, -1)):
            nc = (cx + dx, cy + dy)
            if nc in open_cells and nc not in seen:
                seen.add(nc)
                stack.append(nc)
    return seen


def bar_enclosed(cells, w, h, furniture, start):
    floor = {(x, y) for y in range(h) for x in range(w) if cells[y][x] == S.FLOOR}
    floor |= {(int(f[1]), int(f[2])) for f in furniture}
    bars = {(x, y) for y in range(h) for x in range(w) if cells[y][x] == S.WALL_BARS}
    through = flood_cells(floor | bars, start) & floor
    blocked = flood_cells(floor, start)
    return through - blocked
