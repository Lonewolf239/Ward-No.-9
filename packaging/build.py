...
UPLOAD_CONFIG_PATH = os.path.join(ROOT, "game", "_upload_config.py")


def _write_upload_config():
    url = os.environ.get("WARD9_UPLOAD_URL", "")
    key = os.environ.get("WARD9_UPLOAD_KEY", "")
    with open(UPLOAD_CONFIG_PATH, "w", encoding="utf-8") as f:
        f.write(
            "# Generated by packaging/build.py at build time - see _write_upload_config.\n"
            "# Not checked into git; a source checkout simply doesn't have this file.\n"
            f"UPLOAD_URL = {url!r}\n"
            f"UPLOAD_KEY = {key!r}\n"
        )
...
def main():
    ...
    try:
        ...
        if os.path.exists(UPLOAD_CONFIG_PATH):
            os.remove(UPLOAD_CONFIG_PATH)

    if sys.platform == "darwin":
        mac_binary = os.path.join(dist_dir, APP_SLUG)
        if os.path.exists(mac_binary):
            os.remove(mac_binary)
